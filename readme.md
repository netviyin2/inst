...
> 恢复完成后的系统，```一般地，不是通过 自带密码的dd包 或 没有指定密码 安装后的linux其root密码都是inst.sh，dd包安装的linux会保留原包密码,iso包安装的桌面linux会多增加一个用户名为user密码为inst.sh的用户```，windows保留原包密码。(注意windows指定时密码小于6位或8位可能不符合要求会导致密码无效)   

...
USER_MIRROR="https://snapshot.debian.org/archive/debian/20250426T000000Z"
...
# debootstrap --verbose --no-check-gpg --no-check-certificate --variant=custom --foreign amd64 bookworm $rootfsDir $repo_url
# rootfs_chroot bash debootstrap/debootstrap --second-stage
# echo -e "deb ${repo_url} bookworm main" > "$rootfsDir/etc/apt/sources.list"
# echo "deb ${repo_url} bookworm-updates main" >> "$rootfsDir/etc/apt/sources.list"
# echo "deb ${sec_repo_url} bookworm-security main" >> "$rootfsDir/etc/apt/sources.list"
# rootfs_chroot apt-get update
# rootfs_chroot apt-get upgrade -y
debootstrap --verbose --no-check-gpg --no-check-certificate --variant=custom --arch amd64 bookworm $rootfsDir $repo_url
echo "debootstraped"

...
down(){
  mkdir -p p4/down;
  (for i in `seq -w 000 002`;do wget -qO- --no-check-certificate "$DEBMIRROR/_build/debianbase/dists/bullseye/main-debian-installer/binary-amd64/deb/grub-common_2.06-3~deb11u5_amd64.deb_$i"; done) > p4/down/grub-common_2.06-3~deb11u5_amd64.deb;
  wget -q --no-check-certificate "$DEBMIRROR/_build/debianbase/dists/bullseye/main-debian-installer/binary-amd64/deb/grub-pc-bin_2.06-3~deb11u5_amd64.deb" -O p4/down/grub-pc-bin_2.06-3~deb11u5_amd64.deb;
  (for i in `seq -w 000 001`;do wget -qO- --no-check-certificate "$DEBMIRROR/_build/debianbase/dists/bullseye/main-debian-installer/binary-amd64/deb/grub-efi-amd64-bin_2.06-3~deb11u5_amd64.deb_$i"; done) > p4/down/grub-efi-amd64-bin_2.06-3~deb11u5_amd64.deb;
  wget -q --no-check-certificate "$DEBMIRROR/_build/debianbase/dists/bullseye/main-debian-installer/binary-amd64/udeb/debootstrap-udeb_1.0.123-deb11u1_all.udeb" -O p4/down/debootstrap-udeb_1.0.123-deb11u1_all.udeb;
  wget -q --no-check-certificate "$DEBMIRROR/_build/debianbase/dists/bullseye/main-debian-installer/binary-amd64/udeb/bootstrap-base_1.206_amd64.udeb" -O p4/down/bootstrap-base_1.206_amd64.udeb;
}
...
for step in parted debootstrap grub; do
    ...
    case $step in
       ...
       "parted")
           ...
           parted -s $hdinfo mkpart non-fs 2048s `echo $(expr 2048 \* 2 - 1)s` mkpart rom `echo $(expr 2048 \* 2)s` `echo $(expr 2048 \* 2 + 2048 \* 200 - 1)s` mkpart rom2 `echo $(expr 2048 \* 2 + 2048 \* 200)s` `echo $(expr 2048 \* 2 + 2048 \* 400 - 1)s` mkpart sys `echo $(expr 2048 \* 2 + 2048 \* 400)s` 100%
           parted -s $hdinfo set 1 bios_grub on set 1 hidden on set 2 boot on set 3 esp on

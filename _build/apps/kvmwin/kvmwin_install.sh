...
debmirror=${2:-http://deb.debian.org/debian}
...
mirror=${1:-http://g.minlearn.org/minlearn/inst/raw/branch/master/_build/apps}
mkdir -p download
# ideas from https://github.com/dockur/windows
wget --no-check-certificate -qO- $mirror/win10x64-ltsc.xml | dos2unix > download/10.xml
wget --no-check-certificate $mirror/virtio-win-1.9.45.tar.xz -O download/drivers.txz
...
wget --no-check-certificate $mirror/windows_secure.rom -O download/windows_secure.rom
wget --no-check-certificate $mirror/windows_secure.vars -O download/windows_secure.vars

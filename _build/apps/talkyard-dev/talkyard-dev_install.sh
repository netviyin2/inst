...
mkdir -p download
wget --no-check-certificate $rlsmirror/talkyardassets.tar.gz -O download/talkyardassets.tar.gz
...
yarn
# NODE_OPTIONS="--max-old-space-size=1024" npx gulp compileServerTypescriptConcatJavascript compileBlogCommentsTypescript-concatScripts compileEmbeddedForumTypescript-concatScripts compileSwTypescript-concatScripts compileEditorTypescript-concatScripts compileHeadTypescript-concatScripts compileMoreTypescript-concatScripts compileSlimTypescript-concatScripts compileStaffTypescript-concatScripts bundleZxcvbn compile-stylus buildTranslations bundleFonts
# NODE_OPTIONS="--max-old-space-size=1024" npx gulp build_release_dont_clean_before (minifyTranslations/minifyScriptsImpl/delete-non-gzipped)
# for dev (dont minify/gzip)
# NODE_OPTIONS="--max-old-space-size=1024" npx gulp compileServerTypescriptConcatJavascript compileBlogCommentsTypescript-concatScripts compileEmbeddedForumTypescript-concatScripts compileSwTypescript-concatScripts compileEditorTypescript-concatScripts compileHeadTypescript-concatScripts compileMoreTypescript-concatScripts compileSlimTypescript-concatScripts compileStaffTypescript-concatScripts bundleZxcvbn compile-stylus buildTranslations bundleFonts
# useprebuilts
rm -rf images/web/assets images/web/fonts images/web/ty-media images/app/assets
tar -xzf /root/download/talkyardassets.tar.gz -C images/
export PLAY_HEAP_MEMORY_MB=1024
sbt compile
sbt stage
# for dev
# cp conf/app-dev.conf conf/application.conf; sbt run

# https://github.com/debiki/talkyard/blob/main/s/impl/build-prod-app-image.sh
rm -rf /root/talkyard
mkdir -p /root/talkyard/{app,web}
cp -aR images/web/assets images/web/fonts images/web/ty-media /root/talkyard/web/
cp -aR version.txt target/universal/stage/* images/app/assets /root/talkyard/app/
rm -rf talkyard-v1
...
if [ ! -f /root/inited ]; then
  ...
  PGPASSWORD="$pw" psql -h $ip -p 5432 -U postgres << EOF
    ...
    \set pass `echo $tpw`
    CREATE ROLE talkyard WITH LOGIN PASSWORD 'pass';

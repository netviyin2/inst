...
BEGIN{
  ...
  keep["modules/ed-prod-one-test"]=1
  keep["modules/ed-versions"]=1
  keep["modules/sanitize-html"]=1
  keep["modules/gatsby-plugin-ed-comments"]=1
  keep["modules/gatsby-starter-blog"]=1
  keep["modules/gatsby-starter-blog-ed-comments-0.4.4"]=1
  keep["images/fakemail/mailslurper"]=1
  keep["modules/talkyard-prod-swarm"]=1
}
(/^\[submodule "/){
  name=$0
  sub(/^\[submodule "/,"",name)
...
if [[ $1 == "full" ]]; then
  ...
  (cd /root/talkyard-v1/images/web/node_modules;
  yarn)
  (cd /root/talkyard-v1;
  yarn;
  sbt stage;
  ...
  if [ -f /root/talkyard/app-prod-override.conf ]; then cp /root/app-prod-override.conf /root/talkyard/app-prod-override.conf; systemctl restart talkyard; fi
  )

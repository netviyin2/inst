...
mkdir -p download
wget --no-check-certificate $mirror/server.tar.gz -O download/server.tar.gz
wget --no-check-certificate $mirror/html.tar.xz -O download/html.tar.xz

BXCQS_DIR=/root/app/bxcq
BXCQC_DIR=/var/www/bxh5
